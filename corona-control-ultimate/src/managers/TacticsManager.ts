...
class TacticsManager {
    ...
    // Called by PoliceController to register itself
    public registerController(controller: PoliceController) {
        if (!this.squads.has(1)) this.squads.set(1, []);
        // Avoid duplicate
        const squad = this.squads.get(1);
        if (squad && !squad.includes(controller.getData().id)) {
                squad.push(controller.getData().id);
        }
        
        // Store controller Ref
        this.controllers.set(controller.getData().id, controller);
    }
