...
function calculateSkyColors(gameMinutes: number): {
    ...
    const phase = TimeSystem.getCurrentPhase();
    switch (phase) {
 ...
 */
const DynamicLighting: React.FC<DynamicLightingProps> = ({ quality, castShadows = true }) => {
    const directionalLightRef = useRef<THREE.DirectionalLight>(null);
    const fillLightRef = useRef<THREE.DirectionalLight>(null);
    const rimLightRef = useRef<THREE.DirectionalLight>(null);

    // Shadow Map Größe basierend auf Quality
    ...
    useFrame(() => {
        const gameMinutes = TimeSystem.getCurrentGameMinutes();
        ...
        // V6.0 Laternen Flicker Event
        if (intensity.flicker) {
            // Rapid flicker logic: ~15Hz flicker during the flicker second
            // lampStatus = Math.random() > 0.5;
        }
        ...
        // Broad-casting lamp status could be done via custom event or store
        // For now, we update the local component lighting balance
    });
    ...
    return (
        <>
            ...
            {/* Hauptlicht (Sonne/Mond) */}
            <directionalLight
                ref={directionalLightRef}
                position={[80, 100, 60]}
                intensity={2.0}
                castShadow={castShadows && quality !== 'LOW'}
