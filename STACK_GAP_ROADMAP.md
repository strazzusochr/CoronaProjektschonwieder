...
- external Oracle Cloud, Hugging Face, account, secret, and provisioning steps are mostly not verifiable from the repo alone,
...
| --- | --- | --- | --- |
| Architektur & Voraussetzungen | `partial` | `.godmode_env` exists; `.godmode_env.example`; `ENV_REFERENCE.md`; `START_GODMODE.sh`; `START_GODMODE.ps1`; `openhands/docker-compose.yml`; `n8n/docker-compose.yml` | Repo shows the intended local/Oracle-shaped structure, but account setup, Oracle installation, and access prerequisites from the guide are external. |
...
| Aider — Der Git-Native Code Wizard | `partial` | `hf_aider/README.md`; `aider_godmode.ps1`; `godmode_auto_pilot.py` references `Aider-Cloud` and an HF Space URL | The repo clearly targets Aider integration, but the Oracle installation path from the guide is not fully materialized; current documentation is thin. |
| LangGraph — Der 8-Agent Orchestrator | `partial` | `langgraph/system.py`; `hf_langgraph_space/app.py`; `hf_langgraph/README.md`; `STACK_OPERATIONS.md`; `START_GODMODE.sh` starts a LangGraph API | Repo has a working LangGraph-shaped service, but not the 8-agent orchestrator promised by the guide. Current implementation is a much smaller single-flow oracle node. |
...
6. External dependencies such as Oracle Cloud, Hugging Face Spaces, account provisioning, secrets, and hosted URLs are essential to the guide, but mostly unverifiable from the repo alone.
